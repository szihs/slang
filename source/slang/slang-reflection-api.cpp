    ...
    else if (auto vectorTypeLayout = as<VectorTypeLayout>(typeLayout))
    {
        ...
        if (category == SLANG_PARAMETER_CATEGORY_UNIFORM)
        {
            ...
            if (auto elementType = vectorTypeLayout->type)
            {
                ...
                {
                    if (auto basicElementType =
                            as<BasicExpressionType>(vectorType->getElementType()))
                    {
                        if (basicElementType->getBaseType() == BaseType::Bool)
                        {
                            // Check if this is for a CUDA-like target where bool vectors use int
                            // layout For CUDA, boolean vectors use 4-byte elements
                            // (sizeof(int32_t))

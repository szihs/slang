...
First, fetch the PR diff yourself with `gh pr diff <number>` and the changed files list. Then dispatch each subagent, passing the diff content and changed file list in their prompt. Instruct each to read CLAUDE.md for project context. Subagents can read local files (base branch) for surrounding context but cannot run Bash — you must provide the diff to them. Each should report only noteworthy findings with confidence ≥80.
